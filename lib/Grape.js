...
class Grape extends Events {
  ...
  get (opts, cb) {
    ...
    if (opts.hash) {
      this.mutableGet(opts, (err, res) => {
        ...
        // mutable store didn't have any values, try immutable
        if (res.v === null) {
  ...
  immutablePut (value, cb) {
    try {
      value = typeof value === 'string' ? this.str2buf(value) : value
  ...
  mutablePut (opts, cb) {
    try {
      const { k: publicKey, v, ...options } = opts
      options.keypair = { publicKey }
      if (options.sign) {
        throw Error('ERR_GRAPE_SIGN_NOT_SUPPORTED')
      }
      const value = typeof v === 'string' ? this.str2buf(v) : v
      const qs = this.node.mutable.put(value, options, (err, { key }) => {
        cb(err, key)
      })
      qs.on('warning', this[kOnWarning])
    } catch (e) {
  ...
  immutableGet (hash, cb) {
    ...
    try {
      const qs = this.node.immutable.get(hash, (err, value) => {
        if (err) {
          cb(err)
          return
        }
  ...
  mutableGet (opts, cb) {
    ...
    try {
      ...
      const qs = this.node.mutable.get(_key, options, (err, info) => {
        ...
        const { value, id = '', sig, salt } = info
        const res = {
          id: this.str2hex(id),
          v: value ? value.toString() : value,
          sig: sig ? sig.toString('hex') : sig,
          salt: salt ? salt.toString('hex') : salt
        }
        cb(null, res)
      })
      qs.on('warning', this[kOnWarning])
