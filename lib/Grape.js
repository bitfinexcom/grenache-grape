...
class Grape extends Events {
  ...
  announce (key, port = this.conf.dht_port, cb) {
    ...
    if (!_.isInteger(port)) {
      return process.nextTick(cb, new Error('ERR_GRAPE_SERVICE_PORT'))
    }
  ...
  get (opts, cb) {
    ...
    process.nextTick(cb, Error('ERR_GRAPE_INVALID_GET_OPTIONS'))
  }
  ...
  immutablePut (value, cb) {
    try {
      ...
      process.nextTick(cb, new Error(msg))
    }
  }
  ...
  mutablePut (opts, cb) {
    try {
      ...
      if (options.sign) {
        process.nextTick(cb, Error('ERR_GRAPE_SIGN_NOT_SUPPORTED'))
        return
      }
      ...
      getRawBody(req, { limit: maxPayloadSize }, (err, buf) => {
        if (err) {
          if (err.type === 'entity.too.large') {
            res.statusCode = 413
            res.end('ERR_GRAPE_PAYLOAD_SIZE')
            return
          }
          res.statusCode = 500
          res.end(err.message)
