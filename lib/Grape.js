...
class GenericErrorWrap extends GrapeError {
  constructor (err, code) {
    ...
    super('ERR_GRAPE_GENERIC: ' + err.message, code || err.code)
  }
}
...
class Grape extends Events {
  ...
  onRequest (type, data, cb) {
    ...
    if (!this[met]) {
      return cb(new GrapeError('ERR_REQ_NOTFOUND', 404))
    }
  ...
  get (opts, cb) {
    ...
    process.nextTick(cb, new GrapeError('ERR_GRAPE_INVALID_GET_OPTIONS'))
  }
  ...
  immutablePut (value, cb) {
    try {
      ...
      process.nextTick(cb, new GenericErrorWrap(err, 500))
    }
  }
  ...
  mutablePut (opts, cb) {
    try {
      ...
      if (options.sign) {
        process.nextTick(cb, new GrapeError('ERR_GRAPE_SIGN_NOT_SUPPORTED'))
        return
      }
      ...
      getRawBody(req, { limit: maxPayloadSize }, (err, buf) => {
        if (err) {
          if (err.type === 'entity.too.large') {
            const payloadErr = new GrapeError('ERR_GRAPE_PAYLOAD_SIZE', 413)
            res.statusCode = payloadErr.code
            res.end(payloadErr.toString())
            return
          }
          res.statusCode = 500
          res.end(err.message)
