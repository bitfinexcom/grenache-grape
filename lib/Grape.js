...
const PeerStore = require('bittorrent-dht/peer-store')
...
class Grape extends Events {
  ...
  _maybeGrapeReady () {
    const eventsToHappen = [
      'dht_ready',
      'grape_server_booted'
    ].length
    console.log(this._bootSeq)

    if (eventsToHappen !== this._bootSeq) {
      return
    }

    this.emit('grape-ready')
  }
