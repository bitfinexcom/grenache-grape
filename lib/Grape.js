...
class Grape extends Events {
  ...
  transportHttp (cb) {
    const { maxPayloadSize, api_port } = this.conf
    if (!api_port) return cb(new Error('ERR_NO_PORT'))
    ...
    let fetchRequest = (req, rep) => {
      ...
      req.on('data', (data) => {
        ...
        if (received >= maxPayloadSize) {
          rep.statusCode = 500
          rep.end('ERR_PAYLOAD_MAX_SIZE')
        }
