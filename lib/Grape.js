...
class Grape extends Events {
  ...
  onRequest (type, data, cb) {
    ...
    if (!this[met]) {
      return cb(new GrapeError('ERR_REQ_NOTFOUND'))
    }
  ...
  immutablePut (value, cb) {
    try {
      ...
      process.nextTick(cb, new GrapeError(msg))
    }
  }
  ...
  transportHttp (cb) {
    if (!this.conf.api_port) return cb(new GrapeError('ERR_NO_PORT'))
