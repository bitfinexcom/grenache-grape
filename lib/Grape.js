...
class Grape extends Events {
      ...
      firstGet((err, res) => {
        ...
        if (!res || res.v === null) {
          secondGet((err, res) => {
            ...
            // in an ambiguous mutable/immutable "not found" situation
            // we can't label the data as mutable or immutable because
            // it doesn't exist. Therefore we delete the `m` prop.
            if (res.v === null) delete res.m
            cb(null, res)
          })
        } else {
          cb(null, res)
  ...
  mutablePut (opts, cb) {
    try {
      ...
      if (!sig) {
        process.nextTick(cb, new GrapeError('ERR_GRAPE_SIG_REQUIRED'))
        return
      }
      ...
      const qs = this.node.mutable.put(value, options, (err, result) => {
        if (err) {
          if (err.message === 'ERR_SEQ_LESS_THAN_CURRENT') {
