...
class Grape extends Events {
  ...
  get (opts, cb) {
    ...
    // or mutable
    if (opts.hash) {
      this.immutableGet(opts.hash, (err, res) => {
        if (err) {
          cb(err)
          return
        }
        // immutable store didn't have any values, try mutable
        if (res.v === null) {
          this.mutableGet(opts, cb)
        } else {
          cb(null, res)
  ...
  immutableGet (hash, cb) {
    ...
    try {
      const qs = this.node.immutable.get(hash, (err, value, { id }) => {
        if (err) {
          cb(err)
          return
        }
  ...
  mutableGet (opts, cb) {
    ...
    try {
      ...
      const qs = this.node.mutable.get(_key, options, (err, info) => {
        ...
        const { value, id = '', sig, salt, seq = null } = info
        const res = {
          id: id ? this.str2hex(id) : null,
          v: value ? value.toString() : value,
